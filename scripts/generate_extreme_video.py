#!/usr/bin/env python3
"""
Generate videos of trained models in the LunarLander environment with extreme conditions.
"""
...
def make_env(env_id, wind_prob=0.05):
    """Create environment with extreme settings."""
    def _init():
        try:
            # Try different parameter names based on Gymnasium version
            try:
                # Newer versions use turbulence_power
                env = gym.make(env_id, render_mode="rgb_array", enable_wind=True, wind_power=25.0, turbulence_power=wind_prob)
            except TypeError:
                try:
                    # Older versions might use wind_probability
                    env = gym.make(env_id, render_mode="rgb_array", enable_wind=True, wind_power=25.0)
            ...
            try:
                env = gym.make(fallback_env_id, render_mode="rgb_array", enable_wind=True, wind_power=25.0)
            except TypeError:
...
def generate_extreme_video(model_path, output_path, env_id="LunarLander-v3", episodes=3, vecnorm_path=None):
    """Generate a video of the model in extreme conditions."""
    # Create environment
    env_fn = make_env(env_id, wind_prob=0.05)
    env = env_fn()
    ...
    if vecnorm_path is not None and os.path.exists(vecnorm_path):
        ...
        vec_env = DummyVecEnv([make_env(env_id, wind_prob=0.05)])
...
def main():
    parser = argparse.ArgumentParser(description="Generate videos of trained models in extreme conditions")
